...
{ 
    ...
    vec3 positions[4]; 
    positions[0] = vec3(sin(time * 1.4 / 2.0) * 0.3, cos(time * 2.3 / 2.0) * 0.4, 0.22);
    positions[1] = vec3(sin(time * 3.0 / 2.0) * 0.5, cos(time * 1.3 / 2.0) * 0.3, 0.12);
    positions[2] = vec3(sin(time * 2.1 / 2.0) * 0.1, cos(time * 1.9 / 2.0) * 0.35, 0.4);
    positions[3] = vec3(sin(time * 1.1 / 2.0) * 0.4, cos(time * 2.6 / 2.0) * 0.5, 0.15);
    ...
    //draw 
    vec3 col = 0.5 + 0.5*cos(time*1.5+uv.xyx+vec3(0,2,4)); 
    //vec3 col = vec3(1.);
